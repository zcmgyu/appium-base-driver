...
async function guineaPigTemplate (req, res, page) {
  ...
  if (delay) {
    params.delay = `<link rel="stylesheet" type="text/css" href="/test/delay.css?delay=${delay}">`;
  }
...
async function delayCss (req, res) {
  let delay = parseInt(req.params.delay || req.query.delay || 0);
  if (delay) {
...
export { guineaPig, guineaPigScrollable, welcome, delayCss, STATIC_DIR };
